package com.jshy.check.service.impl;//import org.apache.commons.lang3.StringUtils;
 ...
 */
@Slf4j
public class SensitiveUtils implements SensitiveFilterService {
...
//        // 保留图片标签
//        text = sb.toString().replaceAll("(?!<(img).*?>)<.*?>", "")
//                .replaceAll("(on[^\"]+)", "");
//        // 删除转义字符
//        text = text.replaceAll("&.{2,6}?;", "");
//        // 删除script标签
//        text = text.replaceAll("<[\\s]*?script[^>]*?>[\\s\\S]*?<[\\s]*?\\/[\\s]*?script[\\s]*?>", "");
//        // 删除style标签
//        text = text.replaceAll("<[\\s]*?style[^>]*?>[\\s\\S]*?<[\\s]*?\\/[\\s]*?style[\\s]*?>", "");
